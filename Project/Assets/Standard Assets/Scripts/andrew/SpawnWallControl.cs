...
public class SpawnWallControl : MonoBehaviour {
	...
	void Update () {
		if (!RunPlayerControl.ballHit && SplashScreen.gameGO) {
	...
	public void sheepSpawn () {
		...
		if (obstacleReady) {
			if (first) {
				...
				Instantiate (portalL, new Vector3 (115.79f, 0.0f, originPositionZ), Quaternion.Euler (90.0f, 0.0f, 0.0f));
				Instantiate (portalR, new Vector3 (136.24f, 0.0f, originPositionZ), Quaternion.Euler (90.0f, 0.0f, 0.0f));
				first = false;
				alongX = -2.0f;
			}
	...
	// function that works similiarly to JavaScript
	...
	public void wallSpawn () {
		...
		if (extraBoolean) {
			if (Random.Range (0, 30) <= Random.Range (0, 30) && !RunPlayerControl.wallHit && !RunPlayerControl.ballHit && obstacleReady) {
				Instantiate (wall, new Vector3(loc, 2.3f, player.transform.position.z+distance), Quaternion.identity);
			}
			extraBoolean = false;
		}
	}
