...
public class ScoreControl : MonoBehaviour {
	...
	public static void reloadReset () {
		...
		Debug.Log (TimerControl.gameEnded);
		ShootoutTarget.playerHit = false;
	...
	private void OnGUI () {
		if (showScore) {
			...
			if (!DataContainer.isPlayMode && Application.loadedLevelName == "Whack-A-Mole") {
				scoreCount = masterMoleControl.totalHits;
			}
			else if (!DataContainer.isPlayMode && Application.loadedLevelName == "Run Run") {
				scoreCount = WallControl.wallScore;
			}
            else if (!DataContainer.isPlayMode && Application.loadedLevelName == "Frogger") {
                scoreCount = TimerControl.scoreCount;
            }
            else if (!DataContainer.isPlayMode && Application.loadedLevelName == "TargetPractice") {
                scoreCount = ShootoutShooter.Score;
            }
            else
            {
